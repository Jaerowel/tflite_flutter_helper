...
class TfliteFlutterHelperPlugin : FlutterPlugin, MethodChannel.MethodCallHandler,
    ...
    private fun initRecorder() {
        ...
        mRecorder = AudioRecord(
            MediaRecorder.AudioSource.MIC, mRecordSampleRate,
            AudioFormat.CHANNEL_IN_MONO, mRecordFormat, mRecorderBufferSize
        )
        if (mRecorder != null) {
    ...
    private fun stopRecording(result: MethodChannel.Result) {
        try {
            ...
            debugLog("record() failed")
            result.error(SoundStreamErrors.FailedToRecord.name, "Failed to stop recording", e.localizedMessage)
        }
    }
